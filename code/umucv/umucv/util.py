...
def putText(img, string, orig=(5,16), color=(255,255,255), div=2):
    (x,y) = orig
    if div > 1:
        (w,h), b = cv.getTextSize(string, cv.FONT_HERSHEY_PLAIN, 1, 1)
        img[y-h-4:y+b, x-3:x+w+3] //= div
    cv.putText(img, string, (x,y), cv.FONT_HERSHEY_PLAIN, 1, color, 1, cv.LINE_AA)
...
class Video:
    def __init__(self, ext='mp4', codec='X264'):
        self.ext = ext
        self.codec = codec
        self.fps = deque(maxlen=20)
        self.ON = False
        self.Open = False
        self.nf = 0
    def write(self, frame, key, keypressed):
        if key==keypressed:
            self.ON = not self.ON
