...
class P(Counter):
    ...
    def __add__(self,p):
        return joint(p,self).marginal(sum)
...
def show(p,alpha=1, ticks=False, rotation=0):
    x,y = zip(*sorted(p.items()))
    x1,x2,*_ = x
    try:
        plt.bar(x,y,width=(x2-x1)*1, alpha=alpha,edgecolor='black')
        if ticks: plt.xticks(x)
    except TypeError:
        z = np.arange(len(x))
        plt.bar(z,y,width=1, alpha=alpha,edgecolor='black')
